# Compute several Fibonacci numbers and put in array also reverse the sequence and store in an array, then print
	.data
	fibs:.word   0 : 19	# "array" of words to contain fib values
	rfibs: .word 0 : 19   	# "array" of 12 words to contain reversefibs value
	...
	la   $s0, fibs        	# load address of array
      	...
      	la   $t6, rfibs       	# load address of rfibs to $t6
...
loop: 	lw   $s3, 0($s0)      	# Get value from array F[n-2]
      	...
      	sw   $s2, 64($t6)	# Store newly computed array in R[n]	
      	addi $s0, $s0, 4      	# increment address to now-known Fib. number storage
      	addi $t6, $t6, -4	# decrement address to now known RevFib number storage 
	...
	#Printing Reverse Fibonacci Numbers      
      	la   $a0, head1        	# load address of print heading
      	li   $v0, 4           	# specify Print String service
      	syscall               	# print heading      
      	la   $a0, rfibs        	# first argument for print (array)
      	add  $a1, $zero, $s5  	# second argument for print (size)
      	jal  print            	# call print routine. 
...
head1: 	.asciiz "\n The Reverse Fibonacci numers are:\n"
      	.text
